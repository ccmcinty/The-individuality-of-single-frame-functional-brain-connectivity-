...
% $Id: wfu_find_files.m,v 1.7 2021/05/17 13:41:54 jppittma Exp $

% $Log: wfu_find_files.m,v $
% Revision 1.7  2021/05/17 13:41:54  jppittma
% WFU find files no longer depends on being able to read nifti headers. This is more in line with its name, which is wfu_find_files, not wfu_find_files_print_them_read_niftis_show_header_information.
%
% Revision 1.6  2011/06/21 20:20:23  bkraft
% Limit size of display width.
%
% Revision 1.5  2010/08/06 20:26:34  htan
% *** empty log message ***
%
% Revision 1.4  2010/08/02 20:45:50  bkraft
% Updated functions to display PID.
%
% Revision 1.3  2009/11/17 12:57:31  bkraft
% Superficial changes. Updated comments and output display.
%
% Revision 1.2  2009/11/02 17:01:53  bkraft
% Updated function to list Series Number, Description, and Matrix Size
%
% Revision 1.1.1.1  2009/10/22 16:56:39  bkraft
% Transferring files from ANSIR to LCBN
%
% Revision 1.11  2009/08/26 15:48:31  bkraft
...
defaultOptions = struct('recursionDepth',0,'recursionMaximumDepth',recursionMaximumDepth, ...
                        'displayFiles', false, 'displayDirectories', false, ...
...
if options.displayFiles
    ...
    for ii=1:length(fnames)
        ...
        iiFullFileName = char(fnames{ii});
        [iiDirName iiFileName ] = wfu_fileparts(iiFullFileName, true);
        
        nFullFileName = length(iiFullFileName);
        nFileName     = length(iiFileName);
        
        maxFullFileName = max([maxFullFileName, nFullFileName]);
        maxFileName     = max([maxFileName,     nFileName]);

    end
    
    if( maxFullFileName < maxDisplayWidth )
        
        displayFullDir = true;
        stringPrintF = sprintf('%%2d) %%-%ds', maxFullFileName);
    else
        displayFullDir = false;
        stringPrintF = sprintf('%%2d) %%-%ds', maxFileName+2);
    end
...
for i=1:length(d)
    ...
    if ( ((~d(i).isdir && ~options.findDirectories) || ...
            (d(i).isdir && options.findDirectories)) ...
            && ~isempty(expressionIndices))
        ...
        if numMatches == 1
            ...
            newDirectoryFoundFlag = true;
            for jj=1:numDirectoryMatches
                if strcmp(fileDirectory,dnames{jj})
                    newDirectoryFoundFlag = false;
                    break;
                end
            end
            
            if newDirectoryFoundFlag
    ...
    elseif  (options.recursionMaximumDepth > options.recursionDepth) && d(i).isdir 
